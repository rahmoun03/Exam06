...
int main(int ac, char **av) {
	...
	if ((bind(sockfd, (const struct sockaddr *)&servaddr, sizeof(servaddr))) != 0) ft_error();
	if (listen(sockfd, 10) != 0) ft_error();
		...
		for (int fd = 0; fd <= max; fd++)
		{
			if(FD_ISSET(fd, &cur_read))
			{
				if(fd == sockfd)
				{
					...
					broadcast(newClient, str);
				}
				else
				{
					char buffer[4095];
					int len = recv(fd, buffer, 4095, 0);
					if (len <= 0)
					{
						FD_CLR(fd, &cur);
						char str[100];
